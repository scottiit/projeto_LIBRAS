...
O classificador atual exporta 32 exemplos estáticos do CSV, abrangendo 21 letras. H/J/K/X/Z seguem o fluxo temporal com exemplos de movimento por perfil. H e K saíram dos exemplos estáticos para distinguir rotação do punho e deslocamento da mão. O exportador versiona um hash do CSV e lê as proporções das mídias originais. `x` e `z` são corrigidos pela relação largura/altura antes da normalização pelo tamanho da palma. Sem essa correção, fotos verticais e webcam horizontal produzem geometrias incompatíveis.
...
| `js/dynamic.js` | Segmentação de movimento, exemplos H/J/K/X/Z, DTW e rejeição de sequências incertas |
